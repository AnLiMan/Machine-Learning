...
class ImageView(QWidget):
    def __init__(self, previewImage, fileName):
        ...
        copy_button.setToolTip("Скопировать имя в буфер")
        top_layout.addWidget(copy_button)
        copy_button.clicked.connect(self.copy)
        launch_button = QPushButton("Открть")
        launch_button.setToolTip("Открыть изображение")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        shutter_icon = QIcon(os.path.join(os.path.dirname(__file__),
                            "shutter.svg"))
        self._take_picture_action = QAction(shutter_icon, "&Сделать снимок", self,
                                            shortcut="Ctrl+T",
                                            triggered=self.take_picture)
        self._take_picture_action.setToolTip("Сделать снимок")
        file_menu.addAction(self._take_picture_action)
        tool_bar.addAction(self._take_picture_action)

        exit_action = QAction(QIcon.fromTheme("application-exit"), "В&ыход",
                              self, shortcut="Ctrl+Q", triggered=self.close)
        file_menu.addAction(exit_action)

        about_menu = self.menuBar().addMenu("&Справка")
        about_qt_action = QAction("Справка &о API PyQt", self, triggered=qApp.aboutQt)
        ...
        if self._camera and self._camera.error() == QCamera.NoError:
            ...
            self.setWindowTitle(f"Необработанное изображение с камеры ({name})")
            ...
            self.setWindowTitle("Необработанное изображение с камеры")
            self._take_picture_action.setEnabled(False)
            self.show_status_message("Камера не найдена")
    ...
    def image_saved(self, id, fileName):
        ...
        self._tab_widget.addTab(image_view, f"Снимок #{index}")
